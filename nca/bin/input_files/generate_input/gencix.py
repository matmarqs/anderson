...
#import sys
#sys.path.insert(0, "/home/brunomax/.local/lib/python2.6/site-packages/numpy")
#sys.path.insert(0, "/home/brunomax/.local/lib/python2.6/site-packages/scipy")
...
def cprint(U):
    for i in range(shape(U)[0]):
        for j in range(shape(U)[1]):
            f = U[i,j]
            if abs(f)<1e-10: f = 0j
            print "%7.4f %7.4f*i " % (f.real, f.imag),
        print
    print
...
if __name__ == '__main__':
    ...
    print >> sys.stderr, 'Stage0: Exact diagonalization of the atom'
    ...
    print >> sys.stderr, 'Stage1: Computing F^ in direct base'
    ...
    print >> sys.stderr, 'Stage2: Compressing F^+ according to its block diagonal form'

    for i in range(len(wstates)): wgr[i] = compress(wgr[i])

    print >> sys.stderr, 'Stage3: Renumbers states -- creates superstates for ctqmc'
    ...
    print >> sys.stderr, 'Stage4: F^dagger matrices between superstates evaluated'
                ...
                for ii0,i0 in enumerate(group):
                    for jj0,j0 in enumerate(wgr[ifi][ifi_ins]):
                        ...
                        if abs(FKP[iw][ib][i0,j0].imag)>1e-10:
                            print >> sys.stderr, 'TROUBLE!!! Complex eigenvector!!!'
    ...
    ############################################################################################
    print >> sys.stderr, 'Stage5: Renumber states for oca - each atomic states has unique number'
    # renumbers states such that each of 1024 states has unique index
    # also remembers energy and N for each state
    ...
    foca=open('cix.dat', 'w')
    print >> foca, '# Input file for OCA impurity solver.', 'nlevels=', nlevels, 'Eimp=', Eimp, 'mOCA=', mOCA,  'Eoca=', Eoca
    print >> foca, len(kbth), (("%d "*len(kbth)) % tuple(map(len,kbth))), pl, 0,
    print >> foca, '# Number of impurity levels, their degeneracy and number of local valence and local core states'
    print >> foca, '%3s' % '#',
    print >> foca, ("%6s")*len(kbth) % tuple(map(lambda x: 'N'+str(x), range(len(kbth)))),
    print >> foca, "%4s" % 'Mtot', '%4s' % 'deg', '%12s' % 'Eatom',
    print >> foca, ("%3s"%'#b')*len(back_bubs[i]),
    print >> foca, ("%3s"%'#f')*len(forw_bubs[i])

    # write total spin S of PPs to file
    ppfile=open('ppspin.dat', 'w')

    for i,ii in enumerate(iequiv):
        if pu[i] <0 : continue  # state not used

        print >> ppfile, "%3d" % pu[i], "   ", ipS[ii[0]]
        ...
        for b in range(len(kbth)):
            for ks in back_bubs[i][b]:
                print >> foca, "%10.6f x %-3d  " % (back_bubs[i][b][ks], pu[ks]),

        for b in range(len(kbth)):
            for ks in forw_bubs[i][b]:
                print >> foca, "%10.6f x %-3d  " % (forw_bubs[i][b][ks], pu[ks]),
